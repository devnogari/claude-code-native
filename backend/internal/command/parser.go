...
var (
	...
	// fileIncludeRegex matches @filename patterns (at start of line or after whitespace)
	fileIncludeRegex = regexp.MustCompile(`(?m)(?:^|\s)@([^\s]+)`)
	// bashCommandRegex matches !command at start of line
	bashCommandRegex = regexp.MustCompile(`(?m)^!(.+?)$`)
)
...
func (p *Parser) ProcessFileIncludes(ctx context.Context, content, basePath string, depth int) (string, bool, error) {
	...
	matches := fileIncludeRegex.FindAllStringSubmatchIndex(content, -1)
...
func (p *Parser) ProcessBashCommands(ctx context.Context, content, cwd string) (string, bool, error) {
	matches := bashCommandRegex.FindAllStringSubmatchIndex(content, -1)
	...
	for i := len(matches) - 1; i >= 0; i-- {
		...
		// Execute with timeout - cancel immediately after use to avoid context leak in loop
		execCtx, cancel := context.WithTimeout(ctx, BashTimeout)
		cmd := exec.CommandContext(execCtx, cmdName, cmdArgs...)
		cmd.Dir = cwd

		output, err := cmd.Output()
		cancel() // Cancel immediately, not defer (we're in a loop)
